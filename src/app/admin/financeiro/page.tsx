  ...
  return (
    <div className="p-6 space-y-6 dark:bg-gray-900">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold dark:text-white">Financeiro Administrativo</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
        {/* Receita */}
        <div className="lg:col-span-1 xl:col-span-1 bg-white dark:bg-gray-800 rounded-lg shadow p-6 flex flex-col">
        ...
        <div className="lg:col-span-1 xl:col-span-1 bg-white dark:bg-gray-800 rounded-lg shadow p-6 flex flex-col">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400">Total de Clientes</h2>
              <p className="text-2xl font-bold mt-1 dark:text-white">{financialSummary.currentMonth.totalClients}</p>
            </div>
            <div className="p-2 rounded-full bg-blue-100 dark:bg-blue-900/30">
              <PieChart size={20} className="text-blue-600 dark:text-blue-500" />
            </div>
          </div>
          <div className={`mt-4 text-sm ${clientsChange >= 0 ? 'text-green-600 dark:text-green-500' : 'text-red-600 dark:text-red-500'} flex items-center`}>
            {clientsChange >= 0 ? <ArrowUp size={16} /> : <ArrowDown size={16} />}
            <span className="ml-1">{Math.abs(clientsChange).toFixed(1)}% desde o mês anterior</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-1 xl:col-span-1 bg-white dark:bg-gray-800 rounded-lg shadow p-6 flex flex-col">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400">Novos Clientes (Mês)</h2>
              <p className="text-2xl font-bold mt-1 dark:text-white">{financialSummary.currentMonth.newClients}</p>
            </div>
            <div className="p-2 rounded-full bg-violet-100 dark:bg-violet-900/30">
              <BarChart2 size={20} className="text-violet-600 dark:text-violet-500" />
            </div>
          </div>
          <div className="mt-4 text-sm text-gray-500 dark:text-gray-400">
            Crescimento de 12% em 6 meses
          </div>
        </div>
      </div>
                ...
                filteredTransactions.map((transaction) => (
                  <tr key={transaction.id}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {transaction.client && (
                        <div className="text-xs text-gray-500 dark:text-gray-400">{transaction.client}</div>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
